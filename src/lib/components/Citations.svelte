...
<div class="options">
	{#each citations as item, i}
		<label for={i}>
			<input type="radio" id={i} value={i} name="citation" checked={i === 0 ? true : false} bind:group={selected} /> To
			cite {item.insert}
		</label>
	{/each}
</div>
...
<div id="citation-box" class="text">
	<p id="citation" contentEditable="true" suppressContentEditableWarning={true}>
		{citations[selected].authors}, '{page}', <em>Marking Country</em>, Research Centre for Deep History, Australian National University, {location}, accessed online {date}.
	</p>
