<script>
	...
	onMount(() => {
		// This ensures that when the component mounts, we apply the theme immediately to avoid FOUC.
		const theme = $darkMode ? 'dark' : 'light';
		document.documentElement.setAttribute('data-theme', theme);
	});

	// Reactively watch darkMode changes and apply them. This will not prevent FOUC but ensures
	// that subsequent changes are handled correctly.
	$: {
		const theme = $darkMode ? 'dark' : 'light';
		document.documentElement.setAttribute('data-theme', theme);
	}
...
<svelte:head>
	<script>
		// This IIFE will run before your Svelte component mounts and sets the initial theme based on localStorage or system preference
		(function () {
			const darkModeExists = localStorage.getItem('darkMode');
			const theme =
				darkModeExists === 'true'
					? 'dark'
					: darkModeExists === 'false'
						? 'light'
